...
# Original Author: Connor McGuile
# Feel free to use in any way.
# https://github.com/mcguile/ROS-Dynamic-Window-Approach/blob/master/navigation/src/dwa.py
...
maybe grab config file here:
https://github.com/ROBOTIS-GIT/turtlebot3/blob/main/turtlebot3_description/rviz/model.rviz
...
class DwaCostmapNode(Node):
    def __init__(self):
        ...
        self.sub_odom = self.create_subscription(Odometry, '/odom', self.config.assignOdomCoords,self.qos_profile)
...
def tall_obstacle_marker(rgb_image, centers):
...
class Config:
    ...
    def __init__(self,intensity_publisher,intensity_publisher_high):
        ...
        self.v_x = 0.0
        self.w_z = 0.0
        # note turtlebot3 world x-y axis initialize flipped. robot init pos: : [-2, -0.5]
        self.goalX = 1
        self.goalY = 0.5
        self.th = 0.0
    ...
    def costmap_sum(self):
        ...
        if self.obs_low_mid_high.shape[0] != 0:
            self.costmap_rgb = tall_obstacle_marker(self.costmap_rgb, self.obs_low_mid_high)
        else:
            pass
...
def atGoal(config, x):
    print(f"current x0: {x[0]:.3f} x1: {x[1]:.3f} goalx {config.goalX} goaly {config.goalY}")
    # check at goal
    dist = math.sqrt((x[0] - config.goalX)**2 + (x[1] - config.goalY)**2)
    print(f"dist {dist:.4f} robot radius {config.robot_radius}")
    print(f"returning {math.sqrt((x[0] - config.goalX)**2 + (x[1] - config.goalY)**2) <= config.robot_radius}")
